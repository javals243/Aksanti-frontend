...
export default class TabBar<T: *> extends React.Component<Props<T>, State> {
  ...
  _animate = (index) => {
    const newIndex = (typeof index === "undefined" ) ? this.props.navigationState.index : index;
    const width = this._getTabWidth(this.props);
    const bounceValue = new Animated.Value(this.props.navigationState.index * width);

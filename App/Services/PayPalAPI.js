...
const PAYPAL_URL = Config.PayPal.sandBoxMode
  ? 'https://api.sandbox.paypal.com/v1'
  : 'https://api.paypal.com/v1'
export const RETURN_URL = `${AppConfig.Website.url}/success`
export const CANCEL_URL = `${AppConfig.Website.url}/cancel`
...
const PayPalAPI = {
  startPaymentProcess: (transactions, callback) => {
    const body = Object.assign({}, PAYMENT_BODY, {
      transactions,
    })
    PayPalAPI.getAccessToken((access_token) =>
      PayPalAPI.createPayment(access_token, body, callback)
    )
  },
  createPayment: (access_token, body, callback) => {
    ...
    fetch(`${PAYPAL_URL}/payments/payment`, fetchOptions)
      ...
      .catch((error) =>{
        // console.log(`Error while trying to get payment request: ${error}`)
      })
  },
  executePayment: (executeURL, payer_id, access_token, callback) => {
    ...
    fetch(executeURL, fetchOptions)
      ...
      .catch((error) =>{
        // console.log(`Error while trying to execute PayPal payment: ${error}`)
      })
  },
  getAccessToken: (callback) => {
    ...
    fetch(`${PAYPAL_URL}/oauth2/token`, fetchOptions)
      .then((response) => response.json())
      .then((json) => {
        if (!json.error) {
          // Check if there is an error

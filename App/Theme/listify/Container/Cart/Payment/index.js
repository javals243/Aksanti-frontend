  ...
  nextStep = () => {
    ...
    if (list[this.state.selectedIndex].id === 'cod') {
      ...
      WooWorker.createNewOrder(
        ...
        (json) => {
          let persons = {}
          persons[cartItems[0].product.id] = cartItems[0].product.id
          let booking = {
            productId: cartItems[0].product.id,
            persons,
            date_start: cartItems[0].metaData.filter(
              (item) => item.key == 'Start'
            ),
            date_end: cartItems[0].metaData.filter((item) => item.key == 'End'),
          }
          Api.createPost(user.data, booking, null, json.id)
            .then((response) => {
              //After that, update BookingID to linking with orderID
              WooWorker.setBookingID(json.id, response.post.id, (result) => {
